...
- **py-lint** [error]: All code must pass ruff check (rules E/W/F/I/B/C4/UP, line-length 100). Config in pyproject.toml or .ruff.toml.
- **py-format** [error]: All code must be formatted with Black (line-length 100). Config in pyproject.toml.
- **py-test-coverage** [error]: All modules must have corresponding pytest test files. New code must include tests. Coverage target: 50% (measured via pytest/coverage.py). This target is not yet enforced by CI; maintainers should track progress. Use pytest fixtures for test setup and parameterize for variant testing.
